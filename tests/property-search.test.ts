...
describe('Property Search Tokenizer', () => {
  ...
  describe('Unicode and Emoji Support', () => {
    ...
    it('should evaluate property filter with unicode characters', async () => {
      ...
      let result = await Search.evaluate(
        '[\u00e9cole:français]',
        task,
        false,
        settings,
    ...
    describe('Property Search Evaluator', () => {
      ...
      describe('Basic matching tests', () => {
        it('[type:Project] matches (case insensitive by default)', async () => {
          const task = createTaskWithFrontmatter('test.md', {
            type: 'Project',
          });
      ...
      describe('Exact matching tests', () => {
        ...
        it('["type":"Project"] matches "Project" but not "project"', async () => {
          ...
          const settings = { app: mockApp } as any;
          const result1 = await Search.evaluate(
            '["type":"Project"]',
            task,
            false,
            settings,
          ...
          const result2 = await Search.evaluate(
            '["type":"project"]',
            task,
            false,
            settings,
      ...
      describe('Case sensitivity tests', () => {
        ...
        it('With Match Case disabled: [type:Project] matches "Project" AND "project"', async () => {
          const task1 = createTaskWithFrontmatter('test.md', {
            type: 'Project',
          });
          mockGetFileCache('test.md', { type: 'Project' });
          ...
          const task2 = createTaskWithFrontmatter('test2.md', {
            type: 'project',
          });
      ...
      describe('OR operator within values tests', () => {
        ...
        it('[type:Project OR Task] [status:active] combines OR with other filters', async () => {
          ...
          const settings = { app: mockApp } as any;
          const result = await Search.evaluate(
            '[type:Project OR Task] [status:active]',
            task,
            false,
            settings,
          );
      ...
      describe('Invalid frontmatter tests', () => {
        it('Files with invalid YAML are treated as having no properties', async () => {
          const task = createTaskWithFrontmatter(
            'test.md',
      ...
      describe('Complex expression tests', () => {
        ...
        it('[type:Project] [status:(Draft OR Published)] combines OR in property value', async () => {
          ...
          const settings = { app: mockApp } as any;
          const result = await Search.evaluate(
            '[type:Project] [status:(Draft OR Published)]',
            task,
            false,
            settings,
          );
