...
// Cycle to next state via `todoseq:cycle-task-state`. Obsidian prefixes
// command IDs with the plugin's manifest (`todoseq`), and the cursor sits
// on the task line so the command acts on it instead of the H1.
async function cycleTaskState(page: Page): Promise<void> {
  await page.evaluate(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (line && line.includes('Daily deadline task')) {
...
/** Poll the vault file until the DEADLINE date changes from the given value. */
async function pollDeadlineChanged(
  page: Page,
  filename: string,
  ...
  await expect
    .poll(
      async () => {
        const content = await readVaultFile(page, filename);
        const match = content?.match(/DEADLINE: <(\d{4}-\d{2}-\d{2})/);
...
test.describe('Basic state cycle with DEADLINE + repeat', () => {
  ...
  test('cycle TODO → DOING → DONE resets state and advances DEADLINE', async () => {
    ...
    const deadline1 = await pollDeadlineChanged(
      page,
      FILE,
      initialDateStr,
