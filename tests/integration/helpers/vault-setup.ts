...
export function setupTestVault(): void {
  ...
  fs.writeFileSync(
    path.join(TEST_VAULT_PATH, 'inbox.md'),
    `# Inbox
...
`,
  ...
  fs.writeFileSync(
    path.join(TEST_VAULT_PATH, 'projects', 'alpha.md'),
    `# Project Alpha
...
`,
  ...
  fs.writeFileSync(
    path.join(TEST_VAULT_PATH, 'embedded-demo.md'),
...
`,
  ...
  fs.writeFileSync(
    path.join(TEST_VAULT_PATH, 'daily', `${dateStr}.md`),
    `# ${dateStr}
