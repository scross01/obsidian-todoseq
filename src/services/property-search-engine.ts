...
export class PropertySearchEngine {
  ...
  private async doInitialize(): Promise<void> {
    ...
    try {
      ...
      console.log(`TODOseq: PropertySearchEngine initialize completed in ${duration.toFixed(2)}ms`);
      ...
      console.error('TODOseq: PropertySearchEngine initialization failed:', error);
    } finally {
  ...
  private async waitForVaultScan(): Promise<void> {
    ...
    try {
      const plugin = (window as unknown as { todoSeqPlugin?: any }).todoSeqPlugin;
      if (plugin && plugin.vaultScanner) {
        // Wait for vault scan to complete if it's in progress
        while (plugin.vaultScanner.isScanning() || plugin.vaultScanner.isObsidianInitializing()) {
    ...
    try {
      ...
      if (plugin?.taskStateManager) {
        ...
        if (taskFiles.size > 0) {
          ...
          const isMarkdownFile = file && !('children' in file) && (file as any).extension === 'md';
           ...
           console.log(`TODOseq: PropertySearchEngine scanning ${taskFiles.size} task-containing files`);
          return;
        } else {
      ...
      console.error('TODOseq: Failed to get task files from TaskStateManager:', error);
    }
  }
  ...
  private async buildPropertyCache(key: string): Promise<void> {
    ...
    try {
      ...
      if (plugin?.taskStateManager) {
        ...
        filesToScan = Array.from(taskFiles).map(filePath => {
          const file = this.app.vault.getAbstractFileByPath(filePath);
          // Check if it's a markdown file - type narrow from TAbstractFile to TFile
          // TFile has extension, TFolder has children
          const isMarkdownFile = file && !('children' in file) && (file as any).extension === 'md';
      ...
      console.error('TODOseq: Failed to get task files for property cache:', error);
    }
        ...
        if (taskDate) {
          ...
          if (typeof parsedDate === 'string') {
            ...
            if ('start' in parsedDate && 'end' in parsedDate) {
              // Date range
              if (DateUtils.isDateInRange(taskDate, parsedDate.start, parsedDate.end)) {
                fileSet.forEach(filePath => matchingFiles.add(filePath));
              }
  ...
  private filePathContainsTasks(filePath: string): boolean {
    try {
      ...
      console.error('TODOseq: Failed to check if file path contains tasks:', error);
    }
   ...
   private processPendingUpdates(): void {
    ...
    for (let i = 0; i < pendingUpdatesArray.length; i++) {
      ...
      if (this.app.vault && typeof this.app.vault.getAbstractFileByPath === 'function') {
  ...
  async rebuildAll(): Promise<void> {
    ...
    if (this.startupScanEnabled) {
      try {
        ...
        if (this.pendingUpdates.size > 0) {
          console.log('TODOseq: PropertySearchEngine has pending updates, processing...');
          this.pendingUpdates.clear();
