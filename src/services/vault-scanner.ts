...
export class VaultScanner {
  ...
  constructor(
    ...
    const eventKeys: Array<keyof VaultScannerEvents> = ['tasks-changed', 'scan-started', 'scan-completed', 'scan-error'];
  ...
  on<T extends keyof VaultScannerEvents>(event: T, listener: VaultScannerEvents[T]): void {
    const listeners = this.eventListeners.get(event) || [];
    listeners.push(listener);
    this.eventListeners.set(event, listeners);
  }
  
  off<T extends keyof VaultScannerEvents>(event: T, listener: VaultScannerEvents[T]): void {
    const listeners = this.eventListeners.get(event) || [];
    const filteredListeners = listeners.filter(l => l !== listener);
    this.eventListeners.set(event, filteredListeners);
  }
  
  emit<T extends keyof VaultScannerEvents>(event: T, ...args: Parameters<VaultScannerEvents[T]>): void {
    const listeners = this.eventListeners.get(event) || [];
    listeners.forEach(listener => {
      try {
        listener(...args);
      } catch (error) {
        console.error(`Error in VaultScanner event listener for ${String(event)}`, error);
      }
    });
  }
  ...
  async scanVault(): Promise<void> {
    ...
    try {
      ...
      this.emit('scan-error', error instanceof Error ? error : new Error(String(error)));
    } finally {
  ...
  async handleFileChange(file: TAbstractFile): Promise<void> {
    try {
      ...
      this.emit('scan-error', err instanceof Error ? err : new Error(String(err)));
    }
  }
