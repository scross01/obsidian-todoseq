...
export class DateAutocompleteExtension {
   ...
   */
  private checkLineContext(view: EditorView, currentLineNumber: number): boolean {
    ...
    for (let i = currentLineNumber - 1; i >= 1; i--) {
      ...
      const isTaskLine = taskKeywords.some(keyword =>
      ...
      const isDateLine = prevLineText.startsWith('SCHEDULED:') || prevLineText.startsWith('DEADLINE:');
   ...
   */
  private checkKeywordPosition(lineText: string, cursorOffset: number): boolean {
    const textBeforeCursor = lineText.substring(0, cursorOffset);
 ...
 */
export function dateAutocompleteExtension(settings: TodoTrackerSettings): Extension {
