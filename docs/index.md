...
The plugin also supports tasks in Markdown tables, code block task extraction from 20+ programming languages, state cycling via click interactions, priority markers using `[#A]`, `[#B]`, `[#C]`, full date management with SCHEDULED and DEADLINE dates, and repeating tasks that automatically advance dates when completed.
